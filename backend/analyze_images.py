...
Examples:
  ...
  VISION_LLM_BASE_URL=http://127.0.0.1:8002/v1 \
...
def convert(path:Path, force:bool=False)->Path:
    ...
    with Image.open(path) as im:
        im=im.convert('RGB')
        # Keep the visual token budget safely below a 4096-token model context.
        # 1024 px preserves workshop text reasonably well while avoiding oversized Qwen-VL prompts.
        max_side=1024
...
def analyze_one(path:Path)->dict[str,Any]:
    ...
    if not base or not model:
        raise RuntimeError('Set VISION_LLM_BASE_URL and VISION_LLM_MODEL')
    ...
    with httpx.Client(timeout=180) as c:
        ...
        if r.status_code >= 400:
            raise RuntimeError(f'vLLM HTTP {r.status_code}: {r.text[:2000]}')
...
def main():
    ...
    ap.add_argument('--force-convert',action='store_true',help='Rebuild JPEGs even if cached')
    a=ap.parse_args()
    ...
    if CAT.exists():
        try:
            for x in json.loads(CAT.read_text(encoding='utf-8')):
                existing[x.get('source_image')]=x
        except Exception:
            pass
    for i,p in enumerate(jpgs,1):
        src=p.stem+'.HEIC'
        if src in existing:
            print(f'[{i}/{len(jpgs)}] cached {src}'); continue
    ...
    CHUNKS.write_text(json.dumps([make_chunk(x) for x in items],ensure_ascii=False,indent=2),encoding='utf-8')
